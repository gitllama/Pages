...
class jwafermap{
  ...
  wfenable(){
    ...
    const mark =(x,y,v)=> `<text x=${x+1} y=${y+4} font-size="4">${v}</text>`;

    let start_numX = - this.DisableChip.Left + this.ArixOrigin.X,
        start_numY = - this.DisableChip.Top + this.ArixOrigin.Y,
        end_numX   = start_numX + this.DisableChip.Left + this.DisableChip.Right + this.ShotsNum.X,
        end_numY   = start_numY + this.DisableChip.Top + this.DisableChip.Bottom + this.ShotsNum.Y;
    let convX =(x)=> this.Margin.Left + x * this.ShotSize.X;
    let convY =(y)=> this.Margin.Top + y * this.ShotSize.Y;

    let tags = [];
    tags.push('<g>');
    for(let y = 0; y < this.DisableChip.Top + this.DisableChip.Bottom + this.ShotsNum.Y; y++){
      for(let x = 0; x < this.DisableChip.Left + this.DisableChip.Right + this.ShotsNum.X; x++){
        let ox = x - this.DisableChip.Left + this.ArixOrigin.X,
            oy = y - this.DisableChip.Top + this.ArixOrigin.Y;
        let hoge = this.EnableChips.find(e=> e.X == ox  && e.Y == oy );
        if(!hoge){
          tags.push(rect(
            convX(ox),
            convY(oy)));
    ...
    //TEG
    let hoge = this.EnableChips.filter(e => e.TEG != null);
    tags.push(hoge.map(e =>
        `<rect x=${convX(e.X + this.DisableChip.Left - this.ArixOrigin.X)} `
      + `y=${convY(e.Y + this.DisableChip.Top - this.ArixOrigin.Y)} `
      + `width=${this.ShotSize.X} height=${this.ShotSize.Y} fill="whitesmoke" />`));
  ...
  drawValue(vals){
    ...
    let txtoffset = 9 / 2;
    let ox = this.Margin.Left + (this.DisableChip.Left-this.ArixOrigin.X) * this.ShotSize.X,
        oy = this.Margin.Top + (this.DisableChip.Top-this.ArixOrigin.Y) * this.ShotSize.Y;
...
class RedmineAPI{
  constructor(addr) {
    this.url = `http://${addr}/redmine`;
    this.condition = "";
    this.result = "";
  }

  getTicket(id, callback){
    let _url = `${this.url}/issues/${id}.json?format=json&include=relations,attachments`;
    fetch(_url)
    ...
    .then(json => {
      ...
      for(let i of json.issue.attachments){
        console.log(i);
      }
...
//   callback(data);
//   //console.log(data.issue.attachments[0].content_url)
//   //$.get(data.issue.attachments[0].content_url, e=>{
//   //  console.log(e);
//   //});
// });

// let _url = `http://${addr}/redmine/issues/${ticket}.json?format=json&include=relations,attachments`;
// console.log(_url);
// $.ajax({ type: 'GET', url: _url , dataType: 'json'}).done(data=>{
//   console.log(data.issue.custom_fields[0].value);
//   //let hoge = jsyaml.load(data.issue.custom_fields[0].value);
//
//   this.setState({
//     ticket : data
//   });
